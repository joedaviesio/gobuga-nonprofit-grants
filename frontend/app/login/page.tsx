    ...
    <div className="min-h-screen flex items-center justify-center auth-bg relative overflow-hidden">
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-base text-stone-700 mt-2 font-[family-name:var(--font-dm-sans)]">{t("auth.sign_in_subtitle")}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card-gradient border border-stone-200/60 backdrop-blur-sm p-6 space-y-4 shadow-lg">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="block text-base font-medium text-stone-700">{t("auth.password")}</label>
              <a href="/forgot-password" className="text-sm text-blue-600 hover:underline font-medium">{t("auth.forgot_password")}</a>
            </div>
            <PasswordInput
              value={password}
          ...
          {loading && (
            <LoadingBar label={t("auth.signing_in")} />
          )}
          <button
            type="submit"
            disabled={loading}
