...
function ResetPasswordForm() {
  ...
  if (!token) {
    return (
      <div className="card-gradient border border-stone-200/60 backdrop-blur-sm p-6 text-center shadow-lg">
        <p className="text-base text-stone-800 mb-4">Invalid reset link. No token provided.</p>
        <a href="/forgot-password" className="text-base text-blue-600 hover:underline font-medium">Request a new reset link</a>
      </div>
    );
  }
  ...
  if (success) {
    return (
      <div className="card-gradient border border-stone-200/60 backdrop-blur-sm p-6 text-center shadow-lg">
        <p className="text-base text-stone-800 mb-4">Your password has been reset.</p>
        <a
          href="/login"
  ...
  return (
    <form onSubmit={handleSubmit} className="card-gradient border border-stone-200/60 backdrop-blur-sm p-6 space-y-4 shadow-lg">
      ...
      <div>
        <label className="block text-base font-medium text-stone-700 mb-1">New password</label>
        <PasswordInput
          value={password}
      ...
      <div>
        <label className="block text-base font-medium text-stone-700 mb-1">Confirm password</label>
        <PasswordInput
          value={confirm}
          onChange={setConfirm}
          required
    ...
    <div className="min-h-screen flex items-center justify-center auth-bg relative overflow-hidden">
      ...
      <div className="w-full max-w-sm relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-base text-stone-700 mt-2 font-[family-name:var(--font-dm-sans)]">Set a new password</p>
        </div>

        <Suspense fallback={<div className="card-gradient border border-stone-200/60 p-6 text-center text-base text-stone-600 shadow-lg">Loading...</div>}>
          <ResetPasswordForm />
        </Suspense>
